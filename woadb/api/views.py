...
from api.permissions import IsAdmin, IsAdminOrRead, IsAuthorOrStaffOrReadOnly
...
class SignupAPIView(APIView):
    ...
    def post(self, request):
        ...
        if User.objects.filter(username=username, email=user_email).exists():
            ...
            return Response(
                {"Данный username либо email уже используется"},
                status=status.HTTP_400_BAD_REQUEST
            )
        else:
...
class LoginAPIView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"confirmation_code": 'Введен неверный код!'},
            status=status.HTTP_400_BAD_REQUEST
        )
...
class TitleViewSet(viewsets.ModelViewSet):
    """
    Возвращает список всех произведений, создает,
    ...
    def perform_create(self, serializer):
        ...
        if Review.objects.filter(author=user, title=title).exists():
            raise ValidationError(
                'Нельзя отставлять больше одного отзыва к произведению'
            )
